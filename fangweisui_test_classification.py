...
def detect(number_person):
    ...
    """
    num_classes = 5
    ckpt_file = "./ckpts/ckpt_129_acc90.20.pt"
    net = ResNet_18(num_classes=num_classes)
                    ...
                    if (results[0] > number_person) | (results[1] > number_person):
                        ...
                        if number_person == 1:
                            ...
                            for *xyxy, conf, cls in det:
                                ...
                                ret_image = cv2.resize(ret_image, (224, 224), interpolation=cv2.INTER_LINEAR)

                                # Convert
                                ret_image = ret_image[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
                                ret_image = np.ascontiguousarray(ret_image)
                                ret_image = torch.from_numpy(ret_image)
                                # classes = ["person", "head", "backpack", "hat", "luggage"]
                                classes = ["1", "2", "3", "4", "5"]
                                ret_image = ret_image.to(device).float() / 255.0
                                ret_image = ret_image.unsqueeze(0)
                                out_classify = net(ret_image)
                                _, predict = out_classify.max(1)
                                # print(classes[predict[0]])
                                if save_img or view_img:  # Add bbox to image
                                    if names[int(cls)] == "1" and classes[predict[0]] == "1":
                                        tmpresults[0] += 1
                                    elif names[int(cls)] == "2" and classes[predict[0]] == "2":
                                        tmpresults[1] += 1
                                    elif names[int(cls)] == "1" and classes[predict[0]] == "2":
                                        tmpresults[0] += 1
                                    elif names[int(cls)] == "2" and classes[predict[0]] == "1":
                                        tmpresults[1] += 1
                                    else:
                                        label = '%s %.2f' % (classes[predict[0]], out_classify[0][predict[0]])
                                        plot_one_box_half(xyxy, im0, label=label, color=colors[int(cls)], line_thickness=3)
                                        continue
